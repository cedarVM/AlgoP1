 ...
 *
 * Description:
 * (generally) does the same as quick.last.c except the middle element in
 * a partition is swapped with last before selecting the pivot.
 *
*/
...
int pivot = arr[middle(n)];
arr[middle(n)] = arr[last(n)];
// arr[last(n)] = pivot;
